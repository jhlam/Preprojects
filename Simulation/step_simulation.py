...
#kan bli 20, medium 100, stor 270
step_end = 15
k	= 1 
k_end 	= 20
round_num = 0
...
def initialize():#initialize the simulation
	global g, init_seed, nextg, n, k, infected, boarder, coverage, results, round_num, S, k_counter, round_results,position,step
	...
	for i in range(n):
		g.add_node(i)
	with open("adjacency_small.txt") as f:
		content = f.readlines()		
		for i in range(n):
...
def update():
	global g, nextg, infected, boarder, coverage, n, k_counter, S, k, results, round_num, round_results, step, step_end
	...
	if (len(boarder)==0 or step == step_end):
		print("total stepp: %d \n" %step)
